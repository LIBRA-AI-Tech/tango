...
class Router:
    ...
    def rescale_path(self, path: List[Tuple]) -> List[Tuple]:
        """
    ...
    def orthogonalize_path_line(self, img_map: np.ndarray, start: Tuple[int, int], end: Tuple[int, int]) -> Tuple[Tuple, Tuple]:
        """
    ...
    def orthogonalize_path(self, path: List[Tuple]) -> List[Tuple]:
        """
        ...
        for ((xs,ys), (xd,yd)) in path:
            if xs != xd and ys != yd:
                line1, line2 = self.orthogonalize_path_line(grid_dilated_padded, (xs,ys), (xd,yd))
    ...
    def _get_distance(self, p1: int, p2: int) -> int:
        """Helper function to get the absolute distance between two points"""
        return abs(p1 - p2)

    def _is_same_sign(self, num1: int, num2: int) -> int:
        """Helper function to check if two numbers have the same sign"""
        return num1 * num2 > 0

    def normalize_path(self, path: List[Tuple], threshold: float) -> List[Tuple]:
        """
        ...
        while i < len(path):
            ...
            if point1[0] == point2[0] == point3[0]:
                if self._get_distance(point2[1], point1[1]) >= threshold and not self._is_same_sign(point2[1] - point1[1], point3[1] - point1[1]):
            ...
            # When point1, point2, point3 are on the same horizontal line
                ...
                if self._get_distance(point2[0], point1[0]) >= threshold and not self._is_same_sign(point2[0] - point1[0], point3[0] - point1[0]):
