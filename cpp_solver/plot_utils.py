...
def get_maps(map_parser: Parser, map_type: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray]:  
    """
...
def plot_grid(map_parser: Parser, map_type: Optional[str] = None, draw_plot: bool = True) -> Axes:
    """
    ...
    if map_type is not None:
        image_map, pad_map = get_maps(map_parser, map_type)
...
def plot_map(map_parser: Parser, map_type: str = 'original', draw_plot: bool = True) -> Axes:
    """
    Plot the map.
...
def plot_grid_with_path(map_parser: Parser, path: List[Tuple], map_type: str = 'original', cmap: Optional[str] = None, arrow_dims: Tuple[float,float] = (0.3, 0.3)) -> None:
    """
    ...
    """
    ax = plot_grid(map_parser, map_type, draw_plot=False)
...
def plot_path(map_parser: Parser, path: List[Tuple], map_type: str = 'original', cmap: Optional[str] = None, arrow_dims: Tuple[float,float] = (0.3, 0.3)) -> None:
    """
    ...
    arrow_dims : Tuple[float,float], default=(0.3, 0.3)
        Dimensions of the arrow.
    """
    ax = plot_map(map_parser, map_type, draw_plot=False)
