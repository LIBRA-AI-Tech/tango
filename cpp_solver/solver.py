...
def parse_cell(cell) -> Tuple[int]:
    """
    Parses a cell identifier in the format 'cell-x-y' and extracts the x and y coordinates.

    Parameters
    ----------
        cell (str): The cell identifier string in the format 'cell-x-y'.
...
class Solver():
    ...
    def solve(self, problem, result_status=PlanGenerationResultStatus.SOLVED_OPTIMALLY):
        """
        Solves a given planning problem using a planner and updates the solver's state.

        Parameters
        ----------
            problem (Problem): The problem to solve.
            result_status (PlanGenerationResultStatus, optional): The desired result status.
                Defaults to PlanGenerationResultStatus.SOLVED_OPTIMALLY.

        Returns
        -------
            list: The path generated by the solver.

        Raises
        ------
            ProblemNotSolvable: If the problem cannot be solved.
            NoActionException: If the solver returns no actions in the plan.
        """
